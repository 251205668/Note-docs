...
浏览器将 `cors` 分为两种请求： 简答请求和非简单请求

- 简单请求: 请求方法为 `get`,`post`,`Head` 的一种或者HTTP头部信息不超过以下字段: `Accept`,`Accept-Language`,`Content-Language`
 **简答请求会通过设置 `Origin` 字段，来说明本次请求的是哪个源，进而是否同意访问**
- 非简单请求: 不满足以上条件
  非简单请求的CORS请求，会在正式通信之前，增加一次HTTP查询请求，称为"预检"请求（preflight）。询问请求地址是否在白名单，然后可以使用哪些字段
