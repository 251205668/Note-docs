...
1. `var类型`会有变量提升的情况
2. `let`和`const`没有变量提升的情况，必须要先声明再使用，否则就会出现暂时性死区的情况。
3. `const`和`let`的区别在于一经定义后不得再次改变const定义的值(注意引用类型可能会被问到)
4. `const`声明之后必须赋值
5. 不允许重复声明,例如`let a =1 let a=2`
6. `let` 和 `const` 存在暂时性死区：在代码块内使用const命令和let命令声明变量之前，该变量都不可用
...
JavaScript没有真正的类，一直也是通过函数加原型的形式来模拟，class也不例外，只是语法糖，本质还是函数。
...
[地址](https://github.com/lf2021/Front-End-Interview/blob/master/05.JavaScript/js.md#js%E5%88%A4%E6%96%AD%E5%9B%BE%E7%89%87%E6%98%AF%E5%90%A6%E5%8A%A0%E8%BD%BD%E5%AE%8C%E6%AF%95%E7%9A%84%E6%96%B9%E5%BC%8F)
