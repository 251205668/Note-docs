...
- 不能作为构造函数, `Generator`函数，不能使用new操作符
- 参数不能使用 `arguments` 访问，需要使用Es6的不定参数访问；
- 不可以使用 `bind` 改变this指向
...
- `const`必须赋值
- 不允许重复声明,例如`let a =1 let a=2`
- `let` 和 `const` 存在暂时性死区：在代码块内使用const命令和let命令声明变量之前，该变量都不可用
...
![](https://image.yangxiansheng.top/img/20201211143520.png?imglist)
...
首先执行微任务，执行微任务队列前会去检查当前任务队列是否存在在微任务队列或执行栈任务，若无执行栈任务，则执行完所有的微任务。
...
https://github.com/lf2021/Front-End-Interview/blob/master/05.JavaScript/js.md#js%E5%88%A4%E6%96%AD%E5%9B%BE%E7%89%87%E6%98%AF%E5%90%A6%E5%8A%A0%E8%BD%BD%E5%AE%8C%E6%AF%95%E7%9A%84%E6%96%B9%E5%BC%8F
