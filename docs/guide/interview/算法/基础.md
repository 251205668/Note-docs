...
```
![](https://image.yangxiansheng.top/img/20210312155259.png?imglist)

递归进行一次递归调用，每次递归调用的最大深度为logn。考虑递归最大深度即可

时间复杂度为O(logn)


![](https://image.yangxiansheng.top/img/20210312155955.png?imglist)

一次递归中调用多次递归，这里需要画出递归树进行分析，考虑的是递归调用次数

也就是二叉树的节点个数，O(2^n)

![](https://image.yangxiansheng.top/img/20210312160412.png?imglist)

对于较复杂的情况，也是调用多次递归，这里计算复杂度是通过话出递归树，然后计算层数和每个节点处理的时间复杂度

归并排序和快速排序，一共有logn层，然后每一层需要处理的节点是n个节点，所以时间复杂度为O(nlogn)

![](https://image.yangxiansheng.top/img/20210312162927.png?imglist)
