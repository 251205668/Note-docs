...
所以这道题我们思路可以这样展开
  - **首先初始化一个 `map` 记录满足need子串，初始化所有的 `value` 为1(例如 `T="ABC"`,初始化`Map={'A':1,'B':1,'C':1}`)，初始化一个变量初始值为map的大小(needSize)，初始化返回字符串为空字符串**
...
```
通过记录子问题的结果，递归的时候每种方法只会调用一次。JS可以利用 `Map` 数据结构进行处理。
